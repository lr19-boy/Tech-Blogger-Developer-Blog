...
function Auth() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/auth', {
        ...
        body: JSON.stringify({ isLogin, username, email, password, is_user: isUser }),
      });

      const data = await response.json();
      console.log('Auth response:', { isLogin, isUser, data });
      
      // Only apply validation for login scenarios, not signup
      if (isLogin) {
        if (isUser && data.is_user === false) {
          alert("You cannot log in as admin from the User tab.");
          return;
        }
        if (!isUser && data.is_user === true) {
  ...
  return (
    <div className="auth-container">
      <Helmet>
        <title>Tech Blogger | Auth</title>
        <meta name="description" content="Login or sign up to access Tech Blogger's content and features." />
      </Helmet>
      <TabButtons
        isUser={isUser}
        setIsUser={setIsUser}
